...
do_create_ssh() {
  ...
  echo -ne "  ${YELLOW}Limit Device/IP${NC} [default: ${SESSION_LIMIT_DEFAULT:-2}, 0=unlimited]: "
...
do_info_ssh() {
  ...
  echo -e "  ${YELLOW}Device Aktif    ${NC}: ${WHITE}$active${NC} / ${WHITE}$(fmt_limit "$slimit")${NC}"
...
do_edit_limit_ssh() {
  ...
  echo -ne "  ${YELLOW}Limit Device/IP baru${NC} [saat ini: $(fmt_limit "$cur_slimit")]: "
  ...
  echo -e "  ${GREEN}[✓] Limit '$username' diperbarui: device/IP=$(fmt_limit "$new_slimit"), kuota=$(fmt_quota "$new_quota")${NC}"
  sleep 2
  sshws_menu
}

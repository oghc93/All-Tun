...
trojan_header() {
  ...
  ui_line_center "${WHITE}${BOLD}TROJAN WEBSOCKET${NC}"
  echo -e "${CYAN}$UI_BOX_MID${NC}"
  ui_kv "Domain"      "$domain"
  ui_kv "Status Xray" "$xray_st"
  ui_kv "WS TLS"      "443 - /trojan-ws"
  ui_kv "gRPC TLS"    "443 - trojan-grpc"
...
do_create_trojan() {
  ...
  local pass=$(create_trojan "$username" "$days" "$ip_limit" "$quota_mb" "$trial_hours")
  local domain=$(get_domain)
  local exp masa_label
  if [[ "$trial_hours" -gt 0 ]]; then
    exp=$(get_trojan_info "$username" | cut -d'|' -f3)

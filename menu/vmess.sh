...
vmess_header() {
  ...
  ui_line_center "${WHITE}${BOLD}VMESS WEBSOCKET${NC}"
  echo -e "${CYAN}$UI_BOX_MID${NC}"
  ui_kv "Domain"      "$domain"
  ui_kv "Status Xray" "$xray_st"
  ui_kv "WS TLS"      "443 - /vmess-ws"
  ui_kv "WS nTLS"     "80 - /vmess-ntls"
...
vmess_menu() {
  ...
  ui_2col "$(ui_menu_num 07 'Edit Limit & Kuota')"    "$(ui_menu_num 0 'Kembali' "$DIM")"
  echo -e "${CYAN}$UI_BOX_BOT${NC}"
  echo ""
...
do_create_vmess() {
  ...
  local uuid=$(create_vmess "$username" "$days" "$ip_limit" "$quota_mb" "$trial_hours")
  local domain=$(get_domain)
  local exp masa_label
  if [[ "$trial_hours" -gt 0 ]]; then
    exp=$(get_vmess_info "$username" | cut -d'|' -f3)
...
do_info_vmess() {
  ...
  is_quota_disabled "vmess" "$username" && { sc="${RED}"; st="SUSPENDED (kuota habis)"; }
...
do_edit_limit_vmess() {
  ...
  edit_vmess_limits "$username" "$new_ip" "$new_quota"
